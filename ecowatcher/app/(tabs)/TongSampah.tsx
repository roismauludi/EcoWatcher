...
export default function TongScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Loop through the selected items and send their details including the name
      for (const item of itemsToSubmit) {
        ...
        await fetch(`${CONFIG.API_URL}/api/add-to-tong`, {
          ...
          body: JSON.stringify({
            userId: userId,
            itemId: item.id,
            name: item.name,  // Pastikan 'name' ada
            description: item.description,
            image: item.image,
  ...
  const renderItem = ({ item }: { item: CatalogItem }) => {
    return (
      <View style={styles.itemContainer}>
        ...
        <TouchableOpacity onPress={() => handleShowDetails(item)} style={styles.detailButton}>
  ...
  >
    <View style={styles.modalContainer}>
      <View style={styles.modalContent}>
        ...
        {/* Ganti Button dengan TouchableOpacity */}
        <TouchableOpacity
          style={styles.closeButton} // Tambahkan style untuk tombol
